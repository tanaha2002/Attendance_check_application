 ...
 * @author ADMIN
 */
public class JpnBieuDoNhanVien {
    ...
    private String tenTrucX;
    DefaultCategoryDataset dataset = new DefaultCategoryDataset();
    DefaultPieDataset datasetPie = new DefaultPieDataset();

    public JpnBieuDoNhanVien(javax.swing.JPanel BarChart, javax.swing.JPanel jpnPieChart, String loaiThongKe, String tieuChi1, String tieuChi2) {
        this.BarChart = BarChart;
        this.jpnPieChart = jpnPieChart;
    ...
    public void ThemDuLieuBieuDo(List<List<String>> dsDuLieu) {
        ...
        if (loaiThongKe.equalsIgnoreCase("Thống kê theo năm")) {
            ...
            else if(tieuChi1.equalsIgnoreCase("Số ngày đi làm")){
                tieuDe = "Tổng số ngày đi làm trong năm";
            }
        }
            ...
            if(tieuChi1.equalsIgnoreCase("Lương"))
                ...
                tieuDe = "Tổng số ngày đi làm";
        }
        try {
            for (List<String> duLieu : dsDuLieu) {
            	String baSoCuoi =  duLieu.get(0).substring(5);
            	if(tieuChi2.contains("Top 10")) {
            		tenTrucX = "3 chữ số cuối mã NV";
            		
                    dataset.addValue(Double.parseDouble(duLieu.get(2)), moTa, baSoCuoi);
            	}
            	else {
            		tenTrucX = "Tên nhân viên";
            		dataset.addValue(Double.parseDouble(duLieu.get(2)), moTa, duLieu.get(1));
            	}
            		
                
                //add to pie chart
                datasetPie.setValue(duLieu.get(1) +"(" + baSoCuoi + ")" , Double.parseDouble(duLieu.get(2)));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void HienThiBieuDo()  {
        ...
        JFreeChart barChart = ChartFactory.createBarChart(
                tieuDe, // chart title
                tenTrucX, // domain axis label
                "Lương (VNĐ)", // range axis label
                dataset, // data
...
// Customize the pie chart labels
        ...
        jpnPieChart.add(pieChartPanel, BorderLayout.CENTER);
        jpnPieChart.revalidate();
        jpnPieChart.repaint();
